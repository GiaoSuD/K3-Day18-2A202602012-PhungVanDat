...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            print(f"  [WARN]  OpenAI HyQA failed: {e}")

    # Extractive fallback
    import re
    sentences = [s.strip() for s in re.split(r'[.!?\n]', text) if len(s.strip()) > 10]
    return [f"{s.rstrip('.')}?" for s in sentences[:n_questions]] if sentences else []
...
def contextual_prepend(text: str, document_title: str = "") -> str:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            print(f"  [WARN]  OpenAI contextual failed: {e}")
...
def extract_metadata(text: str) -> dict:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            print(f"  [WARN]  OpenAI metadata failed: {e}")
...
def _enrich_single_call(text: str, source: str) -> dict:
    ...
    [WARN] Cost optimization: 1 API call thay vì 4 calls riêng lẻ.
    """
    if OPENAI_API_KEY:
        try:
            import json as _json
                    ...
                    {"role": "system", "content": """Phân tích đoạn văn và trả về JSON:
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
            ...
            print(f"  [WARN]  Enrichment API failed: {e}")
